...
const form = ref({
  ...
  credit_limit: props.card.credit_limit || '',
...
async function save() {
  if (!form.value.alias.trim()) { error.value = 'Alias is required'; return }
  saving.value = true
  error.value = null
  try {
...
<template>
  <div class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" @click.self="emit('close')">
    <div class="bg-slate-800 rounded-2xl border border-slate-700 w-full max-w-md p-6">
      <h2 class="text-white font-semibold text-lg mb-5">Edit card</h2>
      ...
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            ...
            <input v-model="form.last4" maxlength="4" class="w-full bg-slate-700 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm placeholder-slate-500 focus:outline-none focus:border-indigo-500" />
          </div>
          <div>
            <label class="text-slate-400 text-sm block mb-1">Credit limit</label>
            <input v-model="form.credit_limit" type="number" class="w-full bg-slate-700 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm placeholder-slate-500 focus:outline-none focus:border-indigo-500" />
          </div>
        </div>
        <div>
      ...
      <div class="flex gap-3 mt-6">
        <button @click="emit('close')" class="flex-1 py-2 bg-slate-700 hover:bg-slate-600 text-slate-300 rounded-lg text-sm transition-colors">
          Cancel
        </button>
        <button @click="save" :disabled="saving" class="flex-1 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm transition-colors disabled:opacity-50">
          {{ saving ? 'Saving...' : 'Save' }}
        </button>
      </div>
    </div>
