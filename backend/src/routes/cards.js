...
router.get('/', (req, res) => {
  ...
  if (profile_id) {
    return res.json(db.prepare('SELECT * FROM cards WHERE profile_id=? ORDER BY sort_order ASC, created_at ASC').all(profile_id));
  }
  res.json(db.prepare('SELECT * FROM cards ORDER BY sort_order ASC, created_at ASC').all());
});

// Reorder: body = { ids: ['uuid1', 'uuid2', ...] }
...
router.post('/', (req, res) => {
  ...
  const id = randomUUID();
  db.prepare(
    'INSERT INTO cards (id, bank, alias, last4, credit_limit, color, profile_id) VALUES (?, ?, ?, ?, ?, ?, ?)'
  ).run(id, bank, alias, last4 || null, credit_limit || null, color || '#6366f1', profile_id || null);
